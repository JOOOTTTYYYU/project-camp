...
// Принудительно скроллим наверх сразу
window.scrollTo(0, 0);

document.addEventListener('DOMContentLoaded', function() {
    // Принудительно скроллим наверх при загрузке, если нет якоря в URL
    ...
    setTimeout(() => {
        ...
        // Еще раз убеждаемся, что страница наверху
        if (!window.location.hash || window.location.hash === '#') {
            window.scrollTo(0, 0);
        }
    }, 100);
...
window.addEventListener('load', function() {
    // Убеждаемся, что страница наверху при загрузке (если нет якоря)
    if (!window.location.hash || window.location.hash === '#') {
        window.scrollTo(0, 0);
    }
...
function filterCourses(query) {
    ...
    if (visibleCount === 0 && query !== '') {
        if (!noResults) {
            ...
            noResults.innerHTML = '<p class="text-gray-500 text-lg">Курсы не найдены. Попробуйте другой запрос.</p>';
            document.getElementById('courses-container').appendChild(noResults);
        }
    } else if (noResults) {
        noResults.remove();
    }
